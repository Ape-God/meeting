...
uint32_t RingBuffer::push(void* data, uint32_t size) {
    ...
    memcpy(this->buf + (this->in & (this->size - 1)), data, len);
    /* then put the rest (if any) at the beginning of the buffer */
    memcpy(this->buf, data + len, size - len);
    this->in += size;
    return size;
}
...
uint32_t RingBuffer::pop(void* data, uint32_t size) {
    ...
    memcpy(data, this->buf + (this->out & (this->size - 1)), len);
    /* then get the rest (if any) from the beginning of the buffer */
    memcpy(data + len, this->buf, size - len);
    this->out += size;
